...
def prims_with_prototypes(stage: Usd.Stage) -> tuple[Any, int]:
    """Return ordinary stage traversal plus prototype contents as a generator."""
    prototypes = list(stage.GetPrototypes())
    def _generator():
        yield from stage.Traverse()
        for prototype in prototypes:
            yield from Usd.PrimRange(prototype)
    return _generator(), len(prototypes)
...
def bbox_from_points(points, points_np=None, finite_mask=None) -> tuple[tuple[float, float, float], tuple[float, float, float]] | None:
...
def mesh_record(
    ...
    with phase_timer.phase("mesh.bounds_extent"):
        bounds = bbox_from_points(
            points if points is not None else [],
            points_np=points_np,
            finite_mask=finite_mask if 'finite_mask' in locals() else None,
        )
